package Zvonok.features.auth.jwt.accessToken;

import Zvonok.core.common.exception.customException.accessTokenException.AccessTokenJwtUserNotFoundException;
import Zvonok.core.common.exception.customException.accessTokenException.JwtGenerationException;
import Zvonok.core.common.exception.customException.accessTokenException.AccessTokenJwtSecretException;
...
public class JwtAccessTokenService {
    ...
    public String generateAccessToken(User user) {
        ...
        if (user == null || user.getId() == null) {
            throw new AccessTokenJwtUserNotFoundException("User или ID не могут быть null");
        }
    ...
    private Key getSignInKey() {
        ...
        if (secret == null || secret.isBlank()) {
            throw new AccessTokenJwtSecretException("JWT secret не задан");
        }

        byte[] keyBytes = Decoders.BASE64.decode(secret.trim());

        if (keyBytes.length < 64) {
            throw new AccessTokenJwtSecretException(
